...
*/
__int64 __fastcall DirectBase(__int64 context)
{
  if ( *(_DWORD *)context )
...
""".lstrip(),
            ...
            prototype_stats = report["prototype_correction_stats"]
            self.assertEqual(1, prototype_stats["totals"]["direct_base_deref_survivors"])
            self.assertEqual(1, prototype_stats["top_functions"][0]["direct_base_deref_survivors"])
            body_offset_stats = report["body_offset_residue_review_stats"]
    ...
    def test_parameter_indexed_element_evidence_is_reported(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            function_dir = root / "functions" / "0000000140002000_IndexedElement"
            function_dir.mkdir(parents=True)
            (function_dir / "IndexedElement.cleaned.cpp").write_text(
                """
/*
    Generated by PseudoForge
    Kernel insights:
...
*/
__int64 __fastcall IndexedElement(PMEMORY_RANGE_ENTRY memoryRanges, ULONG_PTR index)
{
  __int64 entry;
...
""".lstrip(),
            ...
            (function_dir / "IndexedElement.ida-batch-summary.json").write_text(
                json.dumps(
                    {
            ...
            self.assertTrue(body_item["cleaned_path"].endswith("IndexedElement.cleaned.cpp"))
            ...
            self.assertTrue(queue_item["cleaned_path"].endswith("IndexedElement.cleaned.cpp"))
            ...
            self.assertIn("parameter-indexed element shape", queue_item["queue_reason"])
