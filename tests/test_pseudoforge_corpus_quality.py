...
""",
    ...
    def test_analyze_corpus_counts_embedded_type_assisted_primary_rewrites(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            function_dir = root / "functions" / "0000000140005000_EmbeddedPrimary"
            function_dir.mkdir(parents=True)
...
void __fastcall EmbeddedPrimary(__int64 context)
{
  context->field_10 /* _QWORD +0x10 */ = 0;
  context->field_18 /* _DWORD +0x18 */ = 1;
  context->field_18 /* _DWORD +0x18 */ += 2;
}
""".lstrip(),
                encoding="utf-8",
            )
            summary_path.write_text(
                json.dumps(
                    {
                        "mode": "ida_batch_export",
                        "function": "EmbeddedPrimary",
                        "function_ea": "0x140005000",
                        "primary_cleaned_source": "type-assisted-preview",
                        "artifacts": {
            ...
            stats = report["layout_rewrite_preview_artifact_stats"]
            self.assertEqual(1, stats["totals"]["preview_artifacts"])
            self.assertEqual(1, stats["totals"]["canonical_rewrite_requested"])
            self.assertEqual(1, stats["totals"]["canonical_rewrite_applied"])
            self.assertEqual(3, stats["totals"]["rewritten_accesses"])
            self.assertEqual(2, stats["totals"]["rewritten_fields"])
            self.assertEqual(1, stats["canonical_rewrite_statuses"]["applied_embedded_primary"])
            self.assertEqual(1, stats["preview_plan_kinds"]["embedded-primary"])
            self.assertEqual("EmbeddedPrimary", stats["top_functions"][0]["name"])
            self.assertEqual("passed_embedded_primary", stats["top_functions"][0]["validation_status"])
    ...
    def test_analyze_corpus_counts_warning_rename_rule_and_text_residue_metrics(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(1, prototype_stats["totals"]["type_assisted_preview_restore_succeeded"])
            self.assertEqual(0, prototype_stats["totals"]["type_assisted_preview_restore_failures"])
            ...
            self.assertEqual(0, prototype_stats["totals"]["idb_apply_parameter_type_corrections"])
            ...
            scorecard = report["structure_quality_score"]
            self.assertEqual("pseudoforge_structure_quality_score_v1", scorecard["schema"])
            self.assertIn("prototype_correctness", scorecard["components"])
            self.assertIn("offset_residue", scorecard["components"])
            self.assertIn("ida_plugin_packaging_boundary", scorecard["components"])
            self.assertEqual("pass", scorecard["hard_gates"]["runtime_boundary"]["status"])
            self.assertEqual("pass", scorecard["hard_gates"]["idb_mutation_default"]["status"])
            self.assertEqual("pass", scorecard["hard_gates"]["type_assisted_preview_restore"]["status"])
            self.assertEqual("missing", scorecard["manual_reread"]["status"])
            self.assertEqual("not_9_yet", scorecard["claim"])
            evidence_graph_stats = report["evidence_graph_stats"]
            self.assertEqual(1, evidence_graph_stats["totals"]["functions_with_evidence_graph"])
            self.assertEqual(4, evidence_graph_stats["totals"]["nodes"])
            self.assertEqual(3, evidence_graph_stats["totals"]["edges"])
            self.assertEqual(0, evidence_graph_stats["totals"]["trusted_rewrite_edges"])
            self.assertEqual(1, evidence_graph_stats["totals"]["trusted_preview_edges"])
            self.assertEqual(1, evidence_graph_stats["totals"]["report_only_edges"])
            self.assertEqual(1, evidence_graph_stats["totals"]["blocked_edges"])
            self.assertEqual(1, evidence_graph_stats["totals"]["rewrite_eligible_edges"])
            self.assertEqual(1, evidence_graph_stats["promotion_lanes"]["trusted-preview"])
            self.assertEqual(1, evidence_graph_stats["promotion_lanes"]["report-only"])
            self.assertEqual(1, evidence_graph_stats["promotion_lanes"]["blocked"])
            self.assertEqual(1, evidence_graph_stats["blockers"]["report_only_profile"])
            self.assertEqual(1, evidence_graph_stats["blockers"]["offset_threshold"])
            self.assertEqual("Sample", evidence_graph_stats["top_functions"][0]["name"])
            self.assertEqual(1, evidence_graph_stats["top_functions"][0]["rewrite_eligible_edges"])
            ...
            self.assertEqual(1, prototype_stats["type_assisted_preview_statuses"]["ok"])
    ...
    def test_structure_quality_scorecard_rewards_typed_structure_cleanup(self) -> None:
        bad_report = _structure_score_report(
            applied_parameter_type_corrections=0,
            blocked_parameter_type_corrections=4,
            function_identity_candidates=0,
            corrected_parameter_map_entries=0,
            body_rewrite_ready=0,
            body_offset_deref_patterns=120,
            body_pointer_indexed_offset_deref_patterns=48,
            body_generic_identifier_tokens=300,
            canonical_rewrite_applied=0,
            layout_validation_errors=2,
            rule_validation_errors=1,
            offset_deref_survivors=120,
            generic_parameter_survivors=80,
            unclassified_offset_residue=3,
        )
        good_report = _structure_score_report(
            applied_parameter_type_corrections=12,
            blocked_parameter_type_corrections=0,
            function_identity_candidates=16,
            corrected_parameter_map_entries=10,
            body_rewrite_ready=8,
            body_offset_deref_patterns=6,
            body_pointer_indexed_offset_deref_patterns=2,
            body_generic_identifier_tokens=18,
            canonical_rewrite_applied=6,
            layout_validation_errors=0,
            rule_validation_errors=0,
            offset_deref_survivors=6,
            generic_parameter_survivors=8,
            unclassified_offset_residue=0,
        )

        bad_score = _structure_quality_scorecard(bad_report)
        good_score = _structure_quality_scorecard(good_report)

        self.assertGreater(good_score["score"], bad_score["score"])
        self.assertGreater(
            good_score["components"]["prototype_correctness"]["score"],
            bad_score["components"]["prototype_correctness"]["score"],
        )
        self.assertGreater(
            good_score["components"]["offset_residue"]["score"],
            bad_score["components"]["offset_residue"]["score"],
        )
        self.assertGreater(
            good_score["components"]["pointer_indexed_residue"]["score"],
            bad_score["components"]["pointer_indexed_residue"]["score"],
        )
        self.assertEqual("fail", bad_score["hard_gates"]["layout_rewrite_validation"]["status"])
        self.assertEqual("pass", good_score["hard_gates"]["layout_rewrite_validation"]["status"])
        self.assertEqual("not_9_yet", good_score["claim"])

    def test_visible_review_only_alias_comments_do_not_inflate_body_residue(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            function_dir = root / "functions" / "0000000140001000_ReviewOnlyAlias"
            function_dir.mkdir(parents=True)
            (function_dir / "ReviewOnlyAlias.cleaned.cpp").write_text(
                """
/*
    Generated by PseudoForge
*/
__int64 __fastcall ReviewOnlyAlias(__int64 context)
{
  if ( ready )
    return 1; // PseudoForge review-only: lastPage.field_18 (base v5, MMPFN, _BYTE, +0x18); no rewrite
  return 0;
}
""".lstrip(),
                encoding="utf-8",
            )
            (function_dir / "ReviewOnlyAlias.ida-batch-summary.json").write_text(
                json.dumps({"function": "ReviewOnlyAlias", "function_ea": "0x140001000"}),
                encoding="utf-8",
            )

            report = analyze_corpus(root)

            self.assertEqual(1, report["body_text_stats"]["visible_review_only_field_alias_annotations"])
            self.assertEqual(1, report["body_text_stats"]["visible_review_only_field_alias_tokens"])
            self.assertEqual(1, report["body_text_stats"]["functions_with_visible_review_only_field_aliases"])
            self.assertEqual(0, report["body_text_stats"]["offset_deref_patterns"])
            self.assertEqual(0, report["body_text_stats"]["generic_identifier_tokens"])
            self.assertEqual(
                "pass",
    ...
    def test_structure_quality_scorecard_fails_type_assisted_restore_mismatch(self) -> None:
        report = _structure_score_report(
            ...
            blocked_parameter_type_corrections=0,
            function_identity_candidates=8,
            corrected_parameter_map_entries=4,
            type_assisted_preview_parameter_corrections=4,
            type_assisted_preview_restore_failures=1,
            body_rewrite_ready=3,
            body_offset_deref_patterns=4,
            body_pointer_indexed_offset_deref_patterns=1,
            body_generic_identifier_tokens=16,
            canonical_rewrite_applied=2,
            layout_validation_errors=0,
            rule_validation_errors=0,
            offset_deref_survivors=4,
            generic_parameter_survivors=4,
            unclassified_offset_residue=0,
        )

        score = _structure_quality_scorecard(report)

        self.assertEqual("fail", score["hard_gates"]["type_assisted_preview_restore"]["status"])
...
/*
    Generated by PseudoForge
    Kernel insights:
      - inferred_offset_stable_base_source: Stable base source for v16: objectHeader (named source, parameter_field_pointer_alias), 12 typed dereference(s) across 12 offset(s). Source detail anchor objectHeader+0x20, type _QWORD, rhs field_pointer. Review-only source identity evidence for temp/generic base promotion. confidence=0.72
...
""".lstrip(),
            ...
            self.assertEqual(
                ...
                item["top_stable_source_details"],
            )
            self.assertEqual(
                {"v16<-objectHeader+0x20:_QWORD:field_pointer": 1},
            ...
            self.assertEqual(
                {"v16<-objectHeader+0x20:_QWORD:field_pointer": 1},
                queue["top_stable_source_details"],
            )
            self.assertIn(
            ...
            self.assertEqual(
                {"v16<-objectHeader+0x20:_QWORD:field_pointer": 1},
                next_candidates["items"][0]["top_stable_source_details"],
            )
            self.assertIn(
                "v16<-objectHeader+0x20:_QWORD",
                next_candidates["items"][0]["source_identity_requirement"],
            ...
            self.assertIn("Residue Stable Source Details", markdown)
            self.assertIn("v16<-objectHeader+0x20:_QWORD", markdown)
            self.assertIn("v16<-objectHeader+0x20:_QWORD:field_pointer", markdown)
...
""".lstrip(),
    ...
    def test_body_offset_residue_splits_source_bound_live_in_parameter_gap_queue(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            function_dir = root / "functions" / "0000000140001000_SourceBoundLiveIn"
            function_dir.mkdir(parents=True)
            diagnostics_path = function_dir / "SourceBoundLiveIn.warning-diagnostics.json"
            cleaned_path = function_dir / "SourceBoundLiveIn.cleaned.cpp"
            summary_path = function_dir / "SourceBoundLiveIn.ida-batch-summary.json"
            diagnostics_path.write_text(
                json.dumps(
                    [
                        {
                            "kind": "unassigned_local_live_in_register",
                            "message": "Uninitialized local risk: v6 appears to be a live-in register value (r9)",
                            "symbol": "v6",
                            "usage": "call argument to MiGetLeafVa",
                            "usage_class": "call_argument",
                            "register": "r9",
                            "register_class": "abi_argument",
                            "candidate_action": "caller_parameter_gap_candidate",
                            "callee_name": "MiGetLeafVa",
                            "argument_index": 0,
                            "confidence": 0.78,
                            "source": "validation.unassigned_local_usage",
                        }
...
/*
    Generated by PseudoForge.
*/
ULONG_PTR __fastcall SourceBoundLiveIn(PMMSUPPORT_INSTANCE workingSet, ULONG_PTR virtualAddress)
{
  __int64 v6; // r9
  ULONG_PTR leafVa;

  leafVa = MiGetLeafVa(v6);
  return *(_QWORD *)(workingSet + 0x28) + leafVa + *(_QWORD *)(workingSet + 0x30);
}
""".lstrip(),
                encoding="utf-8",
            )
                ...
                json.dumps(
                    {
                        ...
                        "function_identity_candidates": [
                            {
                                "profile_id": "windows.memory_manager.source_bound_live_in",
                                "profile_source": "Reviewed exact source-bound live-in parameter gap fixture.",
                                "match_kind": "function_name",
                                "effective_mode": "report-only",
                                "blockers": ["report_only_profile"],
                            }
            ...
            stats = report["body_offset_residue_review_stats"]
            item = stats["top_functions"][0]
            source_bound_queue = stats["review_queues"][
                "source_bound_live_in_parameter_gap_candidates"
            ]
            live_in_queue = stats["review_queues"]["live_in_parameter_gap_candidates"]
            next_item = stats["next_goal_candidates"]["items"][0]
            ...
            self.assertIn(
                "source_bound_live_in_parameter_gap_candidate",
                item["residue_review_notes"],
            )
            ...
            self.assertEqual(
                {"ntoskrnl.exe:26200.8457:x64": 1},
                source_bound_queue["function_identity_source_contexts"],
            )
            self.assertIn(
                "windows.memory_manager.source_bound_live_in",
                source_bound_queue["source_bound_identity_sources"],
            )
            self.assertEqual(
...
/*
    Generated by PseudoForge.
*/
NTSTATUS __fastcall HvReallocateCell(PHHIVE hive, HCELL_INDEX oldCell)
{
  __int64 v12; // rdx

  return *(_BYTE *)(hive + 0x8C) + HvpGetCellFlat(hive, v12);
}
""".lstrip(),
                encoding="utf-8",
            )
            ...
            residue_stats = report["body_offset_residue_review_stats"]
            ...
            self.assertEqual(1, residue_stats["top_functions"][0]["existing_parameter_alias_count"])
            self.assertEqual(
                ["v12(rdx)->oldCell arg1 HvpGetCellFlat"],
                residue_stats["top_functions"][0]["existing_parameter_alias_samples"],
            )
            self.assertEqual(
                1,
                residue_stats["review_queues"]["named_goal_targets"][
                    "existing_parameter_alias_count"
                ],
            )
            self.assertEqual(0, residue_stats["top_functions"][0]["live_in_parameter_gap_count"])
            self.assertEqual(
                0,
                residue_stats["review_queues"]["live_in_parameter_gap_candidates"][
            ...
            self.assertIn("resolved-alias v12(rdx)->oldCell arg1 HvpGetCellFlat", markdown)

    def test_quality_normalizes_generic_prototype_slot_aliases(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            function_dir = root / "functions" / "0000000140229C90_PrototypeAlias"
            function_dir.mkdir(parents=True)
            diagnostics_path = function_dir / "function.warning-diagnostics.json"
            rename_map_path = function_dir / "function.rename-map.json"
            cleaned_path = function_dir / "function.cleaned.cpp"
            summary_path = function_dir / "function.ida-batch-summary.json"
            diagnostics_path.write_text(
                json.dumps(
                    [
                        {
                            "kind": "unassigned_local_live_in_register",
                            "message": "Uninitialized local risk: v33 appears to be a live-in register value (rdx)",
                            "symbol": "v33",
                            "usage": "call argument to ExpGetThreadResourceHint",
                            "usage_class": "call_argument",
                            "register": "rdx",
                            "register_class": "abi_argument",
                            "candidate_action": "caller_parameter_gap_candidate",
                            "callee_name": "ExpGetThreadResourceHint",
                            "argument_index": 1,
                        },
                        {
                            "kind": "unassigned_local_live_in_register",
                            "message": "Uninitialized local risk: v34 appears to be a live-in register value (r8)",
                            "symbol": "v34",
                            "usage": "call argument to ExpGetThreadResourceHint",
                            "usage_class": "call_argument",
                            "register": "r8",
                        ...
                        {
                            "kind": "unassigned_local_live_in_register",
                            "message": "Uninitialized local risk: v35 appears to be a live-in register value (r9)",
                            "symbol": "v35",
                            "usage": "call argument to ExpGetThreadResourceHint",
                            "usage_class": "call_argument",
                            "register": "r9",
...
/*
    Generated by PseudoForge.
*/
__int64 __fastcall PrototypeAlias(__int64 context, char argument1, unsigned __int64 argument2)
{
  __int64 v33; // rdx
  __int64 v34; // r8
  __int64 v35; // r9

  return *(_QWORD *)(context + 0x18) + ExpGetThreadResourceHint(context, v33, v34, v35);
}
""".lstrip(),
                encoding="utf-8",
            )
            summary_path.write_text(
                json.dumps(
                    {
                        "mode": "ida_batch_export",
                        "function": "PrototypeAlias",
                        "function_ea": "0x140229C90",
                        "warnings": 3,
                        "artifacts": {
                            "cleaned_pseudocode": cleaned_path.name,
                            "warning_diagnostics": diagnostics_path.name,
                            "rename_map": rename_map_path.name,
                            "summary": summary_path.name,
                        },
                    }
                ),
            ...
            self.assertEqual(2, item["existing_parameter_alias_count"])
            self.assertEqual(1, item["live_in_parameter_gap_count"])
            self.assertEqual(
                [
                    "v33(rdx)->argument1 arg1 ExpGetThreadResourceHint",
                    "v34(r8)->argument2 arg2 ExpGetThreadResourceHint",
                ],
                item["existing_parameter_alias_samples"],
            )
            self.assertEqual(
                "a2",
                report["existing_parameter_alias_stats"]["top_functions"][0]["aliases"][0][
                    "existing_parameter_raw_name"
                ],
            )
            self.assertEqual(
                "argument1",
                report["existing_parameter_alias_stats"]["top_functions"][0]["aliases"][0][
                    "existing_parameter_rendered_name"
                ],
            )
            self.assertIn(
                "resolved-alias=v33(rdx)->argument1 arg1 ExpGetThreadResourceHint",
                item["review_summary"],
            )
            self.assertEqual(
                [
    ...
    def test_cli_writes_json_and_markdown_reports(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn(
                "- Trusted preview edges: `1` across `1` functions",
                (output_dir / "corpus-quality.md").read_text(encoding="utf-8"),
            )
            self.assertIn(
    ...
    def test_manual_reread_evidence_passes_quality_hard_gate(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir) / "corpus"
            _write_quality_fixture(root)
            ...
            report = analyze_corpus(
                root,
                manual_reread={
                    "inspected_functions": 80,
                    "misleading_rewrites": 0,
                    "improved_functions": 42,
                    "honest_blocked_functions": 38,
                },
            )

            manual = report["structure_quality_score"]["manual_reread"]
            hard_gate = report["structure_quality_score"]["hard_gates"]["manual_reread"]

            self.assertEqual("pass", manual["status"])
            self.assertEqual(80, manual["inspected_functions"])
            self.assertEqual(0, manual["misleading_rewrites"])
            self.assertEqual("pass", hard_gate["status"])

    def test_cli_loads_manual_reread_json_with_item_counts(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir) / "corpus"
            output_dir = Path(temp_dir) / "quality"
            manual_path = Path(temp_dir) / "manual-reread.json"
            _write_quality_fixture(root)
                ...
                json.dumps(
                    {
                        "manual_reread": {
                            "items": (
                                [{"classification": "improved"} for _ in range(45)]
                                + [{"classification": "honest_blocked"} for _ in range(35)]
                            )
                        }
                    },
                    indent=2,
            ...
            self.assertEqual("pass", manual["status"])
            self.assertEqual(80, manual["inspected_functions"])
            self.assertEqual(45, manual["improved_functions"])
            self.assertEqual(35, manual["honest_blocked_functions"])
            self.assertEqual(0, manual["misleading_rewrites"])


def _structure_score_report(
    *,
    applied_parameter_type_corrections: int,
    blocked_parameter_type_corrections: int,
    function_identity_candidates: int,
    corrected_parameter_map_entries: int,
    body_rewrite_ready: int,
    body_offset_deref_patterns: int,
    body_pointer_indexed_offset_deref_patterns: int,
    body_generic_identifier_tokens: int,
    canonical_rewrite_applied: int,
    layout_validation_errors: int,
    rule_validation_errors: int,
    offset_deref_survivors: int,
    generic_parameter_survivors: int,
    unclassified_offset_residue: int,
    type_assisted_preview_parameter_corrections: int = 0,
    type_assisted_preview_restore_failures: int = 0,
) -> dict[str, object]:
    return {
        "totals": {"summaries": 10},
        "api_semantic_stats": {"rejections": blocked_parameter_type_corrections},
        "rule_stats": {"load_errors": 0, "validation_errors": rule_validation_errors},
        "prototype_correction_stats": {
            "totals": {
                "function_identity_candidates": function_identity_candidates,
                "applied_parameter_type_corrections": applied_parameter_type_corrections,
                "blocked_parameter_type_corrections": blocked_parameter_type_corrections,
                "corrected_parameter_map_entries": corrected_parameter_map_entries,
                "type_assisted_preview_parameter_corrections": type_assisted_preview_parameter_corrections,
                "type_assisted_preview_restore_failures": type_assisted_preview_restore_failures,
                "body_rewrite_ready": body_rewrite_ready,
                "generic_parameter_survivors": generic_parameter_survivors,
                "idb_apply_parameter_type_corrections": 0,
            }
        },
        "layout_hint_stats": {"totals": {"functions_with_hints": function_identity_candidates}},
        "layout_stable_base_source_stats": {
            "totals": {"functions_with_source_comments": max(0, function_identity_candidates // 2)}
        },
        "layout_rewrite_ready_stats": {"totals": {"ready_candidates": body_rewrite_ready}},
        "layout_rewrite_preview_stats": {"totals": {"preview_plans": body_rewrite_ready}},
        "layout_rewrite_preview_artifact_stats": {
            "totals": {
                "validation_errors": layout_validation_errors,
                "canonical_rewrite_errors": 0,
                "canonical_rewrite_applied": canonical_rewrite_applied,
            }
        },
        "pointer_indexed_offset_stats": {
            "totals": {
                "pointer_indexed_layout_rewrite_candidates": canonical_rewrite_applied,
                "pointer_indexed_rewrite_applied": canonical_rewrite_applied,
            }
        },
        "body_offset_residue_review_stats": {
            "totals": {
                "functions_with_offset_residue": 1 if offset_deref_survivors else 0,
                "offset_deref_survivors": offset_deref_survivors,
                "generic_parameter_survivors": generic_parameter_survivors,
            },
            "review_classes": {"unclassified_offset_residue": unclassified_offset_residue},
        },
        "body_text_stats": {
            "offset_deref_patterns": body_offset_deref_patterns,
            "pointer_indexed_offset_deref_patterns": body_pointer_indexed_offset_deref_patterns,
            "generic_identifier_tokens": body_generic_identifier_tokens,
        },
    }
        ...
        json.dumps(
            {
                ...
                "evidence_graph": {
                    ...
                    "nodes": [
                        ...
                        {
                            "id": "identity:delete-device",
                            "kind": "function_identity",
                            "label": "windows.io_manager.delete_device",
                            "confidence": 0.88,
                            "profile_id": "windows.io_manager.delete_device",
                            "effective_mode": "report-only",
                            "blockers": ["report_only_profile"],
                        ...
                        {
                            "id": "comment:rewrite-blocker",
                            "kind": "inferred_offset_rewrite_blockers",
                            "label": "sessionSpace",
                            "confidence": 0.73,
                            "blockers": ["offset_threshold"],
                        },
                    ],
                    "edges": [
                        {
                            "source": "function:sample",
                            "target": "identity:delete-device",
                            "kind": "matched_function_identity",
                            "confidence": 0.88,
                            "promotion_lane": "report-only",
                            "rewrite_eligible": False,
                            "blockers": ["report_only_profile"],
                        },
                        {
                            "source": "identity:delete-device",
                            "target": "parameter:device-object",
                            "kind": "corrected_parameter",
                            "confidence": 0.82,
                            "promotion_lane": "trusted-preview",
                            "rewrite_eligible": True,
                            "blockers": [],
                        },
                        {
                            "source": "function:sample",
                            "target": "comment:rewrite-blocker",
                    ...
                    "summary": {
                        ...
                        "blockers": {
                            "offset_threshold": 1,
                            "report_only_profile": 1,
                        },
                        "trusted_rewrite_edges": 0,
                        "report_only_edges": 1,
                        "blocked_edges": 1,
                    },
                },
                ...
                "type_assisted_preview": {
                    ...
                    "error": "",
                    "proposal": {
                        "prototype": "void __stdcall Sample(PDEVICE_OBJECT deviceObject)",
                        "profile_ids": ["windows.io_manager.delete_device"],
                        "evidence": [
                            "function_identity:windows.io_manager.delete_device:exact_function_name"
                        ],
                        "blockers": [],
                        "corrections": [
                            "param0 a1->deviceObject __int64->PDEVICE_OBJECT"
                        ],
                    },
                    "improved_pseudocode_chars": 128,
                    "cleaned_pseudocode_chars": 96,
                    "original_type": "__int64 __fastcall(__int64 a1)",
                    "restored_type": "__int64 __fastcall(__int64 a1)",
                },
                "artifacts": {
