...
"""
        ...
        self.assertEqual("domain_identity_report_only", partial[0]["source_provenance"])
        self.assertEqual("resource", partial[0]["source"])
        self.assertEqual("test.report_only_overlay_named", partial[0]["domain_profile_id"])
        self.assertEqual("domainContext", partial[0]["domain_role"])
        self.assertEqual("TEST_DOMAIN_CONTEXT", partial[0]["domain_structure"])
        ...
        self.assertIn("Source provenance domain_identity_report_only from resource", partial[0]["text"])
...
"""
        ...
        self.assertIn("Source detail anchor context+0x8, type _QWORD, rhs field_pointer", sources[0]["text"])
        self.assertEqual([], blockers)
        self.assertEqual(1, len(ready))
...
"""
        ...
        self.assertEqual(["savedSessionSpace"], stability[0]["stable_post_access_reload_rhs"])
        self.assertEqual(["a2", "savedSessionSpace"], stability[0]["risky_post_access_rhs"])
        self.assertIn("stable reload RHS savedSessionSpace", stability[0]["text"])
        self.assertIn("risky RHS a2; savedSessionSpace", stability[0]["text"])
        self.assertFalse(any(item.get("kind") == "inferred_offset_rewrite_ready" for item in comments))
...
"""
        ...
        self.assertEqual(["savedSessionSpace"], stability[0]["risky_post_access_rhs"])
        ...
        self.assertIn("risky RHS savedSessionSpace", stability[0]["text"])
        self.assertFalse(any(item.get("kind") == "inferred_offset_rewrite_ready" for item in comments))
...
"""
    ...
    def test_call_result_temporary_merge_masks_offset_source_in_comment(self) -> None:
        comments = field_layout_comments(
            """
__int64 __fastcall CallResultTemporaryOffsetSource(__int64 referencedObject, int useCall)
{
  __int64 v20;
  __int64 v29;

  v29 = *(_QWORD *)(referencedObject + 40);
  v20 = ExAllocateFromLookasideListEx(&CcSharedCacheMapLookasideList);
  if ( useCall )
  {
    v20 = *(_QWORD *)(v29 + 8);
...
"""
        ...
        self.assertEqual(1, len(provenance))
        temporary = provenance[0]["temporary_root_candidates"][0]
        self.assertEqual("*(_QWORD *)(referencedObject + 40)", temporary["stable_source"])
        self.assertNotIn("*(_QWORD *)(referencedObject + 40)", provenance[0]["text"])
        self.assertIn("deref(_QWORD,referencedObject@0x28)", provenance[0]["text"])
...
"""
    ...
    def test_alias_equivalent_initializers_before_layout_access_do_not_block_rewrite(self) -> None:
        comments = field_layout_comments(
            """
__int64 __fastcall AliasEquivalentInitializerLayout(__int64 a1)
{
  __int64 v22;
  __int64 v44;
...
"""
        ...
        self.assertEqual("v22", sources[0]["base"])
        self.assertEqual("a1", sources[0]["source"])
        self.assertEqual("v44", sources[0]["source_alias"])
        self.assertEqual("temporary_parameter_direct_alias", sources[0]["source_provenance"])
        self.assertEqual(1, sources[0]["base_alias_assignments"])
        self.assertEqual(1, len(ready))
        self.assertEqual("v22", ready[0]["base"])
        self.assertEqual("a1", ready[0]["source"])
        self.assertEqual("v44", ready[0]["source_alias"])
        self.assertEqual("temporary_parameter_direct_alias", ready[0]["source_provenance"])

    def test_alias_different_initializers_before_layout_access_stay_blocked(self) -> None:
        comments = field_layout_comments(
            """
__int64 __fastcall AliasDifferentInitializerLayout(__int64 a1, __int64 a2)
{
  __int64 v22;
  __int64 v44;
...
"""
    ...
    def test_same_parameter_family_merge_reports_review_disposition(self) -> None:
        comments = field_layout_comments(
            """
__int64 __fastcall SameParameterFamilyMergeLayout(__int64 eaBuffer)
{
  __int64 v22;

  v22 = eaBuffer;
  v22 = *(_QWORD *)(eaBuffer + 8);
  return *(_QWORD *)(v22 + 16)
       + *(_QWORD *)(v22 + 24)
       + *(_QWORD *)(v22 + 32)
       + *(_QWORD *)(v22 + 40)
...
"""
        ...
        self.assertIn("base has multiple initializers before layout access", blockers[0]["blockers"])
        self.assertEqual(1, len(merge))
        self.assertEqual(["eaBuffer", "*(_QWORD *)(eaBuffer + 8)"], merge[0]["source_candidates"])
